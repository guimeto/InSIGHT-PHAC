...
climatology = xr.concat(climatologie_epiweek, dim='time')
...
df_year_epiweek = xr.concat(datasets, dim='time') 
anomalies = ((df_year_epiweek / climatology)-1)*100
...
for name in shapes['NAME']:
    ...
    if np.max(mask) == 0 :
        ...
        for t in range(0,len(anomalies.time.values)-1):
            anomalies_point = anomalies.sel(time=0).sel(longitude=float(loni)  , latitude=float(lati)  , method='nearest')
            climatology_point = climatology.sel(time=t, longitude=float(loni)  , latitude=float(lati)  , method='nearest')        
        ...
        for t in range(0,len(anomalies.time.values)-1):
            anomalies_mask = anomalies.sel(time=t).where(mask == 1)
            climatology_mask = climatology.sel(time=t).where(mask == 1)
